...
import { createViewTransitionProxy, } from './switchable-view-transition.js';
...
*/
export function mayStartViewTransition(param, ext = {}) {
    let { collisionBehavior = 'skipOld', speedUpWhenChained = 1, respectReducedMotion = true, useTypesPolyfill = 'never', } = ext;
    collisionBehaviors.includes(collisionBehavior) ||
        (console.warn(`Invalid collisionBehavior "${collisionBehavior}" specified, using "skipOld" instead`),
    ...
    let surrogate = false;
    if ((collisionBehavior === 'skipNew' && currentViewTransition) ||
    ...
    if (!currentViewTransition || collisionBehavior === 'skipOld') {
        ...
        currentViewTransition = polyfilledTypes(surrogate
            ...
            : document.startViewTransition(unchainUpdates), useTypesPolyfill === 'always' || (useTypesPolyfill !== 'never' && nativeSupport === 'partial'));
...
async function unchainUpdates() {
    ...
    keepLast = false;
    const rejected = (await Promise.allSettled(current.map((u) => u()))).find((r) => r.status === 'rejected');
    if (rejected)
